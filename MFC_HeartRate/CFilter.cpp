...
void CFilter::add_index_range(vectori &indices, int beg, int end, int inc = 1)
{
...
void CFilter::add_index_const(vectori &indices, int value, size_t numel)
{
	while (numel--)
		indices.push_back(value);
}

void CFilter::append_vector(vectord &vec, const vectord &tail)
{
	vec.insert(vec.end(), tail.begin(), tail.end());
}

vectord CFilter::subvector_reverse(const vectord &vec, int idx_end, int idx_start)
{
...
void CFilter::filter(vectord B, vectord A, const vectord &X, vectord &Y, vectord &Zi)
{
	if (A.empty())
	...
	size_t filter_order = A.size() > B.size() ? A.size() : B.size();
...
void CFilter::filtfilt(vectord B, vectord A, const vectord &X, vectord &Y)
{
...
bool CFilter::compare(const vectord &original, const vectord &expected, double tolerance = DBL_EPSILON)
{
